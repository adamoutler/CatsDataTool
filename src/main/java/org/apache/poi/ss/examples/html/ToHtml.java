...
public class ToHtml {
    ...
    public static void main(String[] args) throws Exception {
        args = new String[]{"/var/www/baby-sharks/parts/cats-parts.xlsx", "/var/www/baby-sharks/parts/foo.html"};
    ...
    private void printStyle(CellStyle style) {
        ...
        out.format(".%s .%s {%n", "excelDefaults", styleName(style));
        styleContents(style);
        out.format("}%n");
    }
    ...
    public void printSheet(Sheet sheet) {
        ...
        out.format("<div><h1>" + sheet.getSheetName() + "</h1></div>");
        out.format("<table class=%s style=\"width:%dpx;\">%n", "excelDefaults", tableWidth);
        printCols(widths);
...
//                    row.getRowNum() + 1);
            for (int i = firstColumn; i < endColumn; i++) {
                ...
                if (row.getRowNum() == 0) {
                    out.format("    <td class=myRowHeader %s>%s</td>%n",
                            attrs, content);

                } else if (i==0){
                    if (row.getRowNum() %2==0){
                    out.format("    <td class=myColHeaderEven %s>%s</td>%n", attrs, content);
                    } else {
                    out.format("    <td class=myColHeaderOdd %s>%s</td>%n", attrs, content);    
                    }
                }  else {
                    if (row.getRowNum() %2==0){
                         out.format("    <td class=%s %s>%s</td>%n", styleName(style).equals("style_00")?"style_00even {\n" +
"":styleName(style),
                            attrs, content);
                    } else {
                         out.format("    <td class=%s %s>%s</td>%n", styleName(style).equals("style_00")?"style_00odd {\n" +
"":styleName(style),
                            attrs, content);
                    }
